...
从 https://nodejs.org 下载 LTS 版（≥20），安装时勾选 "Add to PATH"。
...
从 https://ffmpeg.org/download.html 下载 Windows 版本，解压后将 `bin` 目录添加到系统 PATH。

或者在管理员 PowerShell 中用 winget：
```powershell
...
或指定 Windows 分支：
```cmd
git clone -b main https://github.com/Huanuyn1/Galcode.git
```
...
这会在 `%USERPROFILE%\.local\bin\` 下创建 `galcode.bat`。确保该目录在 PATH 中：
```cmd
...
这会生成一个 30 秒的离线 demo 并录制成 MP4。视频在 `outputs\` 下：
```cmd
...
**Q: 提示 `node` 找不到？**
确保 Node.js 已安装且在 PATH 中。运行 `node --version` 验证。

**Q: 录制时卡住不动？**
检查 FFmpeg 是否安装：`ffmpeg -version`。如果没有，用 winget 安装。
